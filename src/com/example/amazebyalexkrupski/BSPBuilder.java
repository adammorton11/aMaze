			...
			if (getSign(dot1) != getSign(dot2)) {
				if (dot1 == 0)
					...
					if (dx == 0)
						...
						spy = y;
					Seg sps1 = new Seg(se.x, se.y, spx-se.x, spy-se.y, se.dist, colchange, maze);
					Seg sps2 = new Seg(spx, spy, sendx-spx, sendy-spy, se.dist, colchange, maze);
					if (dot1 > 0) {
						rsl.add(sps1);
						lsl.add(sps2);
					} else {
		...
		for (x = 0; x != width; x++) {
			...
			while (y < height) {
				...
				sl.add(new Seg(x*Constants.MAP_UNIT, starty*Constants.MAP_UNIT,
						0, (y-starty)*Constants.MAP_UNIT, dists.getDistance(x, starty), colchange, maze));
			}
			y = 0;
			while (y < height) {
				// find the beginning of a segment
				...
				sl.add(new Seg((x+1)*Constants.MAP_UNIT, y*Constants.MAP_UNIT,
						0, (starty-y)*Constants.MAP_UNIT, dists.getDistance(x, starty), colchange, maze));
			}
		}
	}
	/**
		...
		for (y = 0; y != height; y++) {
			...
			while (x < width) {
				...
				sl.add(new Seg(x*Constants.MAP_UNIT, y*Constants.MAP_UNIT,
						(startx-x)*Constants.MAP_UNIT, 0, dists.getDistance(startx, y), colchange, maze));
			}
			...
			while (x < width) {
				...
				sl.add(new Seg(startx*Constants.MAP_UNIT, (y+1)*Constants.MAP_UNIT,
						(x-startx)*Constants.MAP_UNIT, 0, dists.getDistance(startx, y), colchange, maze));
			}
		}
	}
