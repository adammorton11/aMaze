...
import com.example.amazebyalexkrupski.BSPBranch;
import com.example.amazebyalexkrupski.BSPLeaf;
...
import com.example.amazebyalexkrupski.DefaultViewer;
import com.example.amazebyalexkrupski.FloatPair;
import com.example.amazebyalexkrupski.MazeBuilder;
import com.example.amazebyalexkrupski.RangePair;
import com.example.amazebyalexkrupski.RangeSet;
import com.example.amazebyalexkrupski.Seg;
...
public class FirstPersonDrawer extends DefaultViewer {
	 ...
	 */
	public FirstPersonDrawer(Context c,int width, int height, int map_unit, int step_size, Cells mazecells, Cells seencells, int map_scale, int[][] mazedists, int mazew, int mazeh, BSPNode bsp_root, Maze maze){
		view_width = width ;
		view_height = height ;
	...
	public void redraw(MazePanel mazePanel, int state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		...
		if (state != Constants.STATE_PLAY)
			return ;
		// new adjustment
		maze.panel.gc = mazePanel.getBufferGraphics() ;
		this.rset = rset ;
		...
		//mazePanel.gc.setColor(mazePanel.getBlack());
		//Rect bot =  new Rect(0,0,view_width, view_height/2);
		Rect top =  new Rect(0,0,400, 200);
		Bitmap imagetop = BitmapFactory.decodeResource(c.getResources(), R.drawable.starsky);
		maze.panel.gc.drawBitmap(imagetop, null, top, null);
		
		// draw dark gray background on upper half
		
		
		//mazePanel.gc.setColor(mazePanel.getDarkGray());
		//Rect top = new Rect(0, view_height/2, view_width, view_height/2);
		Rect bot = new Rect(0, 200, 400, 400);
		
		Bitmap imagebot = BitmapFactory.decodeResource(c.getResources(), R.drawable.tronflr);
		maze.panel.gc.drawBitmap(imagebot, null, bot, null);
		// set color to white and draw what ever can be seen from the current position
		
		
		//NOW USE 	mazepanel.col AS THE CURRENT PAINT COLOR.
	...
	private boolean boundingBoxIsVisible(int ymax, int ymin, int xmin, int xmax) {
		...
		int[] p = maze.panel.getPoint(x1, x2);
	...
	private void drawSegment(Seg seg, int ox1, int y1, int ox2, int y2) {
		...
		//maze.panel.gc.drawPaint(pain);
		maze.panel.gc.drawPaint(maze.panel.col);
		//Paint paint = new Paint();
		//paint.setColor(Color.RED);
		//maze.panel.gc.drawPaint(paint);
		...
		while (x1i <= x2) {
			...
			int[] p = maze.panel.getPoint(x1i, x2);
			if (!rset.intersect(p))
				break;
			x1i = p[0];
			int x2i = p[1];
			...
			//
			//
			//Here, make a polygon, then fill it with mazePanel.col which is the current color
			//
			//
			...
			poly.close();
			maze.panel.col.setStyle(Style.FILL);
			Paint paint2 = new Paint();
			paint2.setColor(Color.BLUE);
			paint2.setStyle(Style.FILL);
	////////////////////////////////
			final BitmapShader shader;
			final Bitmap fillbmp;
			
			fillbmp = BitmapFactory.decodeResource(c.getResources(), R.drawable.tronwall);
			shader = new BitmapShader(fillbmp, Shader.TileMode.REPEAT, Shader.TileMode.REPEAT);
			
			paint2.setShader(shader);
			maze.panel.gc.drawPath(poly, paint2);
			
			//maze.panel.gc.drawPath(poly, paint2);
			//maze.panel.gc.fillPolygon(xps, yps, 4);
			drawn = true;
			rset.remove(x1i, x2i);
			x1i = x2i+1;
