...
public class MapDrawer extends DefaultViewer {
	...
	public void redraw(MazePanel mazePanel, int state, int px, int py,
		...
		if (state != Constants.STATE_PLAY)
			return ;
		if (maze.isInMapMode()) {
			draw_map(maze.panel, px, py, walk_step, view_dx, view_dy, maze.isInShowMazeMode(), maze.isInShowSolutionMode()) ;
			draw_currentlocation(maze.panel, view_dx, view_dy) ;
		}
	}
	/**
	...
	public void draw_map(MazePanel panel, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		//SET panel.col TO WHITE, THEN USE panel.col WHENEVER A COLOR IS NEEDED.
		panel.col = maze.panel.getWhite();
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				Paint map = new Paint();
				map.setColor((seencells.hasWallOnTop(x, y) ? Color.WHITE : Color.GRAY)) ;
				if ((seencells.hasWallOnTop(x, y) || showMaze) && w)
					panel.gc.drawLine(nx1, ny1, nx2, ny1, map);
				...
				map.setColor((seencells.hasWallOnLeft(x, y) ? Color.WHITE : Color.GRAY));
				if ((seencells.hasWallOnLeft(x, y) || showMaze) && w)
					panel.gc.drawLine(nx1, ny1, nx1, ny2, map);
			}
		if (showSolution) {
	...
	public void draw_currentlocation(MazePanel panel, int view_dx, int view_dy) {
		...
		RectF oval = new RectF(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz + (ctrx-cirsiz/2), cirsiz+(ctry-cirsiz/2));
		panel.gc.drawOval(oval, location);
		//panel.gc.fillOval(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz);
		...
		panel.gc.drawLine(ctrx, ctry, aptx, apty, location);
		panel.gc.drawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy, location);
		panel.gc.drawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy, location);
	}
	...
	public void draw_solution(MazePanel panel, int offx, int offy, int px, int py) {
		...
		panel.col = (maze.panel.getYellow());
		...
		while (d > 1) {
			...
			// do the graphics
			panel.gc.drawLine(nx1, ny1, nx1+ndx, ny1+ndy, sol);
