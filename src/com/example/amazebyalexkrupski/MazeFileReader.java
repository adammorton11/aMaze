...
public class MazeFileReader {
	 ...
	 */
	public MazeFileReader(String filename, Context c) {
		load(filename, c) ;
	}
	 ...
	 */
	public void load(String filename, Context c)
	{
		...
		try{
			...
			if (file.exists()){
			fXmlFile = c.openFileInput(filename);
	...
	private static Seg readSegment(Element eElement, int number, int i) {
		...
		Seg result = new Seg(x,y,dx,dy,dist,cc, null) ;
		// get a few more attributes and set those explicitly
		int col = getElementIntValue("colSeg_" + number+ "_" + i, eElement);
		//
		//
		//The line below used to be .... = new Color(col);  So if there are issues, check here. This might be the problem. 
		//
		//
		//result.mazepanel.col = new Paint(col); 
	...
	private static void compareSegments(ArrayList<Seg> slist, ArrayList<Seg> slist2) {
		...
		for (int i = 0 ; i < n ; i++)
		{
			...
			//if(s.mazepanel.col.getColor() != s2.mazepanel.col.getColor()) 
